...
hexo.extend.filter.register('after_generate', function (locals) {
  ...
  const config = hexo.config.electric_clock ? hexo.config.electric_clock : hexo.theme.config.electric_clock
      console.log(urlFor(config.clock_css))
  // 如果配置开启
  ...
  // 集体声明配置项
    const data = {
      ...
      weather_city: {
        zh: config.weather_city_zh || '北京',
        en: config.weather_city_en || 'beijing'
      }
    }
  // 渲染页面
  const temple_html_text = config.temple_html ? config.temple_html : pug.renderFile(path.join(__dirname, '../layout/clock_html.pug'),data)
  //cdn资源声明
    //样式资源
